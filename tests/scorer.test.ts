...
import {
  generateOutfits, colorHarmonyScore, silhouetteScore, seasonScore,
  formalityScore, occasionScore, preferenceScore, pairScent,
  type ClosetItem, type Fragrance,
...
const T = (over: Partial<ClosetItem> = {}): ClosetItem => ({
  id: over.id ?? crypto.randomUUID(), name: "Item", category: "top", kind: "clothing",
  color: "black", material: "cotton", fit: "relaxed", season: "all", formality: "casual", brand: null, image_url: null, ...over,
});
...
describe("outfit-generator scorers", () => {
  ...
  it("occasion signals surface", () => {
    const good = occasionScore([T({ name: "Oxford shirt", brand: "brand", material: "chino" })], "work");
  ...
  it("rotation returns different sets", () => {
    const closet = [
      ...
      T({ id: "b1", category: "bottom" }), T({ id: "b2", category: "bottom" }), T({ id: "b3", category: "bottom" }),
    ];
    const a = generateOutfits(closet, { occasion: "errands", vibe: "x", temperatureF: 65, dressCode: "casual" }, new Set(), [], new Set(), new Set(), 3, 0);
    const b = generateOutfits(closet, { occasion: "errands", vibe: "x", temperatureF: 65, dressCode: "casual" }, new Set(), [], new Set(), new Set(), 3, 1);
    expect(a.map((x) => x.top.id + x.bottom.id).join()).not.toBe(b.map((x) => x.top.id + x.bottom.id).join());
  });
});
...
describe("scent pairing", () => {
  it("picks a scent by season and family", () => {
    const scents: Fragrance[] = [
      { id: "1", name: "Cold Woody", brand: null, family: "woody", season: "winter", projection: "strong", longevity: "long", occasions: [] },
      { id: "2", name: "Summer Fresh", brand: null, family: "citrus", season: "summer", projection: "soft", longevity: "short", occasions: [] },
    ];
    const pick = pairScent(
      { top: T(), bottom: T({ category: "bottom" }), outerwear: null, shoes: null, accessory: null, score: 1,
        breakdown: { color: 1, silhouette: 1, weather: 1, formality: 1, occasion: 1, preference: 1, diversity: 1 }, rationale: [] },
      scents,
...
describe("shop gap scorer", () => {
  it("flags duplicates", () => {
    ...
    const item: CatalogItem = {
      id: "x", name: "Nike shoe", brand: "Nike", category: "shoes", color: "black",
      price: 100, condition: "new", image_url: null, formality: "casual", season: "all",
    };
  ...
  it("rewards filling thin categories", () => {
    const item: CatalogItem = {
      id: "y", name: "Chinos", brand: "X", category: "bottom", color: "khaki",
      price: 60, condition: "new", image_url: null, formality: "smart_casual", season: "all",
    };
