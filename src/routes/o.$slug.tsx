...
function OutfitPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const assets = await getPublicOutfitAssets({ data: { slug } }).catch(() => ({
        cover: null,
        pieces: [] as Array<{
          id: string;
          name: string;
      ...
      if (authorIds.length) {
        ...
        setCommentAuthors(
          Object.fromEntries(
            (ap ?? []).map((p) => [p.id, { name: p.display_name ?? "someone", handle: p.handle }]),
          ),
        );
      }
      setLoading(false);
  ...
  async function toggleLike() {
    ...
    if (liked) {
      ...
      const { error } = await supabase
        .from("outfit_likes")
        .insert({ outfit_id: outfit.id, user_id: meId });
  ...
  async function postComment() {
    ...
    const { data, error } = await supabase
      .from("outfit_comments")
      .insert({ outfit_id: outfit.id, user_id: meId, body })
      .select("*")
      .single();
    if (error) return toast.error(error.message);
    ...
    if (!commentAuthors[meId]) {
      ...
      if (me)
        setCommentAuthors((a) => ({
          ...a,
          [meId]: { name: me.display_name ?? "you", handle: me.handle },
        }));
    }
  }
    ...
    <div className="min-h-dvh bg-background">
      <div className="container-app py-6">
        ...
        <div className="mt-6 overflow-hidden rounded-2xl border border-border bg-surface-2">
          {coverUrl ? (
            <img src={coverUrl} alt={outfit.name ?? "outfit"} className="w-full object-cover" />
          ) : (
        ...
        <div className="mt-4">
          ...
          {outfit.explanation && (
            <p className="mt-3 text-sm text-foreground/85">{outfit.explanation}</p>
          )}
        </div>
        ...
        <div className="mt-4 flex gap-2">
          <button
            onClick={toggleLike}
            className={`flex-1 flex items-center justify-center gap-1.5 rounded-full border py-2.5 text-xs uppercase tracking-widest ${liked ? "border-primary bg-primary/10 text-primary" : "border-border"}`}
          >
            <Heart className={`h-4 w-4 ${liked ? "fill-current" : ""}`} /> {likeCount}
          </button>
          <button
            onClick={toggleSave}
            className={`flex-1 flex items-center justify-center gap-1.5 rounded-full border py-2.5 text-xs uppercase tracking-widest ${saved ? "border-primary bg-primary/10 text-primary" : "border-border"}`}
          >
          ...
          <button
            onClick={share}
            className="flex-1 flex items-center justify-center gap-1.5 rounded-full border border-border py-2.5 text-xs uppercase tracking-widest"
          >
              ...
              {pieces.map((p) => (
                <li key={p.id} className="card-surface overflow-hidden">
                  ...
                  <div className="p-2">
                    ...
                    <p className="mt-0.5 line-clamp-1 text-[9px] uppercase tracking-widest text-muted-foreground">
