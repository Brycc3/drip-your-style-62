...
function ShopPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: cat }, ci, fb] = await Promise.all([
        supabase
          .from("shop_catalog")
          .select("id,name,brand,category,color,price,condition,image_url,formality,season")
  ...
  const scored: GapScore[] = useMemo(() => {
    const filtered = catalog.filter(
      (c) => (source === "all" || c.condition === source) && !dismissed.has(c.id),
    );
            ...
            <li key={g.item.id} className="card-surface overflow-hidden">
              <div className="grid grid-cols-[7rem_1fr] gap-3">
                ...
                <div className="p-3 pl-0 pr-3">
                  ...
                  {g.item.price != null && (
                    <p className="mt-1 font-display text-lg text-primary">${g.item.price}</p>
                  )}
                  {g.duplicate && (
