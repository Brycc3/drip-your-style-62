...
function primaryKind(cat: string): PrimaryTab | "other" {
  ...
  if (["accessory","hat","cap","beanie","belt","watch","chain","bracelet","ring","bag","sunglasses","tie","socks","grill","scarf","wallet","jewelry"].includes(c)) return "accessories";
  if (["fragrance", "scent", "perfume", "cologne"].includes(c) || /fragrance|perfume|cologne|scent/.test(c)) return "fragrance";
  if (["top","tee","hoodie","shirt","polo","bottom","trousers","cargos","joggers","shorts","denim","pants","outerwear","bomber","chore","jacket","coat"].includes(c)) return "clothing";
  return "other";
}

function primarySlot(cat: string): "top" | "bottom" | "outerwear" | "shoes" | "accessory" | "other" {
  const c = cat.toLowerCase();
  ...
  if (["shoes","sneaker","jordan","vomero","new_balance","loafer","boot","runner"].includes(c)) return "shoes";
  if (["accessory","hat","cap","beanie","belt","watch","chain","bracelet","ring","bag","sunglasses","tie","socks","grill","scarf","wallet","jewelry"].includes(c)) return "accessory";
  return "other";
}
...
function ShopPage() {
    ...
    (async () => {
      ...
      const [{ data: cat }, ci, fb] = await Promise.all([
        supabase.from("shop_catalog")
          .select("id,name,brand,category,color,price,current_price,original_price,condition,image_url,formality,season,retailer,buy_url,availability,is_demo,created_at")
          .order("created_at", { ascending: false })
          .limit(200),
        u ? supabase.from("closet_items")
        ...
        u ? supabase.from("shop_feedback").select("catalog_id, saved, dismissed").eq("user_id", u)
      ...
      try {
        sessionStorage.setItem(CACHE_KEY, JSON.stringify({ catalog: nextCatalog, closet: nextCloset }));
      } catch { /* ignore */ }
  ...
  const closetSummary = useMemo(() => {
    ...
    const targets: Array<[string, number, string]> = [
      ["top", 5, "tops"], ["bottom", 4, "bottoms"], ["outerwear", 2, "outerwear"],
      ["shoes", 3, "shoes"], ["accessory", 3, "accessories"],
    ];
    const gaps = targets.filter(([slot, tgt]) => (counts[slot] ?? 0) < tgt).map(([, , label]) => label);
  ...
  const outfitIdeas = useMemo(() => {
    ...
    return scored.filter((g) => g.matches.length > 0).slice(0, 12)
    ...
    <div className="space-y-5">
      ...
      <div className="-mx-5 flex gap-2 overflow-x-auto px-5 scrollbar-hide">
        {PRIMARY_TABS.map((t) => (
          <button
            key={t.v}
            onClick={() => { setTab(t.v); setVisible(INITIAL); setAccSub("all"); setFragFam("all"); }}
            className={`shrink-0 inline-flex items-center gap-1 rounded-full border px-4 py-1.5 text-xs uppercase tracking-widest ${
              tab === t.v ? "border-primary bg-primary text-primary-foreground" : "border-border text-foreground/80 hover:bg-surface-2"
            }`}
          >
            {t.v === "outfits" ? <Sparkles className="h-3.5 w-3.5" /> : t.v === "accessories" ? <Layers className="h-3.5 w-3.5" /> : null}
            {t.l}
          </button>
        ))}
      </div>
      ...
      {tab === "accessories" && (
        <div className="-mx-5 flex gap-2 overflow-x-auto px-5 scrollbar-hide">
          {ACC_SUBS.map((s) => (
            <button key={s.v} onClick={() => { setAccSub(s.v); setVisible(INITIAL); }}
              className={`shrink-0 rounded-full border px-3 py-1.5 text-[11px] uppercase tracking-widest ${
                accSub === s.v ? "border-primary bg-primary text-primary-foreground" : "border-border text-foreground/80"
              }`}>{s.l}</button>
          ))}
        </div>
      )}
      {tab === "fragrance" && (
        <div className="-mx-5 flex gap-2 overflow-x-auto px-5 scrollbar-hide">
          {FRAG_FAMS.map((s) => (
            <button key={s.v} onClick={() => { setFragFam(s.v); setVisible(INITIAL); }}
              className={`shrink-0 rounded-full border px-3 py-1.5 text-[11px] uppercase tracking-widest ${
                fragFam === s.v ? "border-primary bg-primary text-primary-foreground" : "border-border text-foreground/80"
              }`}>{s.l}</button>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="-mx-5 flex flex-1 gap-2 overflow-x-auto px-5 scrollbar-hide">
          {CONDITIONS.map((s) => (
            <button key={s} onClick={() => { setSource(s); setVisible(INITIAL); }}
              className={`shrink-0 rounded-full border px-3 py-1.5 text-[11px] uppercase tracking-widest ${
        ...
        <div className="flex shrink-0 items-center gap-2">
          <select value={sort} onChange={(e) => setSort(e.target.value as "gap" | "new")}
      ...
      {loading ? (
        ...
        outfitIdeas.length === 0 ? (
          ...
          <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {outfitIdeas.map(({ pick, owned }) => (
              <OutfitCard key={pick.item.id} g={pick} owned={owned} closetUrls={closetUrls}
                saved={saved.has(pick.item.id)} onSave={() => feedback(pick.item.id, "save")} />
            ))}
          </ul>
        )
      ) : scored.length === 0 ? (
        ...
        <>
          ...
          <ul className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {visibleScored.map((g) => (
              <ItemCard key={g.item.id} g={g} saved={saved.has(g.item.id)}
          ...
          {visible < scored.length && (
            <div className="flex justify-center pt-2">
              <button onClick={() => setVisible((n) => n + PAGE_SIZE)}
...
function ItemCard({
  g, saved, onSave, onDismiss,
}: { g: GapScore; saved: boolean; onSave: () => void; onDismiss: () => void }) {
  ...
  return (
    <li className="card-surface overflow-hidden flex flex-col">
      <div className="aspect-square bg-surface-2 relative">
        {g.item.image_url ? (
          <img src={g.item.image_url} alt={g.item.name} loading="lazy" width={480} height={480} className="h-full w-full object-cover" />
        ) : null}
      ...
      <div className="p-3 flex-1 flex flex-col gap-2">
        ...
        {(g.item.current_price ?? g.item.price) != null && (
          <p className="font-display text-lg text-primary">
            ...
            {g.item.original_price && g.item.current_price && g.item.original_price > g.item.current_price && (
              <span className="ml-2 text-xs text-muted-foreground line-through">${g.item.original_price}</span>
            )}
          </p>
        )}
        {g.duplicate && (
...
function OutfitCard({
  g, owned, closetUrls, saved, onSave,
}: {
  g: GapScore; owned: ClosetItem[]; closetUrls: Record<string, string>;
    ...
    <li className="card-surface overflow-hidden">
      ...
      <div className="grid grid-cols-5 gap-1 px-3">
        <div className="col-span-2 aspect-square bg-surface-2 relative rounded-md overflow-hidden">
          {g.item.image_url ? (
            <img src={g.item.image_url} alt={g.item.name} loading="lazy" className="h-full w-full object-cover" />
          ) : null}
        ...
        <div className="col-span-3 grid grid-cols-2 gap-1">
          {owned.map((o) => (
            <div key={o.id} className="aspect-square bg-surface-2 relative rounded-md overflow-hidden">
              {closetUrls[o.id] ? (
                <img src={closetUrls[o.id]} alt={o.name} loading="lazy" className="h-full w-full object-cover" />
              ) : (
      ...
      <div className="p-3 space-y-2">
        ...
        {(g.item.current_price ?? g.item.price) != null && (
          <p className="font-display text-lg text-primary">${g.item.current_price ?? g.item.price}</p>
        )}
