...
const OCCASION_PROFILES: Record<Occasion, OccasionProfile> = {
  work: {
    ...
    keywords: ["oxford", "chino", "loafer", "blazer", "button", "trouser"],
    penalize: ["graphic", "cargo", "athletic"],
  },
  date: {
    formality: "smart_casual",
    keywords: ["leather", "knit", "suede", "denim", "tee"],
    penalize: ["athletic", "fleece"],
  },
  church: {
    formality: "business",
    keywords: ["dress", "oxford", "loafer", "trouser", "blazer"],
    penalize: ["graphic", "athletic", "cargo", "short"],
  },
  brunch: {
    formality: "casual",
    keywords: ["linen", "knit", "denim", "loafer", "tee"],
    penalize: ["athletic"],
  },
  gym: {
    ...
    penalize: ["leather", "oxford", "suede", "denim"],
  },
  errands: { formality: "casual", keywords: ["tee", "hoodie", "denim", "sneaker"], penalize: [] },
  party: {
    formality: "casual",
    keywords: ["graphic", "leather", "denim", "jordan", "boot"],
    penalize: ["oxford"],
  },
  travel: {
    formality: "casual",
    keywords: ["fleece", "tech", "cotton", "sneaker", "joggers", "hoodie"],
    penalize: ["leather", "suede"],
  },
  formal: {
    formality: "formal",
    keywords: ["dress", "oxford", "suit", "wool", "trouser", "blazer"],
    penalize: ["graphic", "athletic", "cargo", "denim", "sneaker", "tee"],
  },
  outdoor: {
    formality: "casual",
    keywords: ["nylon", "fleece", "cargo", "boot", "tech", "hike", "trail"],
    penalize: ["suede", "oxford"],
    needsOuter: true,
  },
};
...
export function colorHarmonyScore(colors: (string | null)[]): { score: number; note: string } {
  ...
  if (new Set(nonN).size === 1) return { score: 0.75, note: `tonal look built around ${nonN[0]}` };
...
export function silhouetteScore(
  ...
  if (isLooseT && isTightB)
    return { score: 1, note: "loose top over slim bottom — balanced silhouette" };
  if (isTightT && isLooseB)
    return { score: 1, note: "fitted top over relaxed bottom — balanced silhouette" };
...
export function seasonScore(
  ...
  return {
    ...
    note:
      ratio === 1
        ? `everything works at ${temperatureF}°F`
        : `${hit}/${items.length} pieces match ${temperatureF}°F`,
  };
}

export function formalityScore(
  items: ClosetItem[],
  target: string,
): { score: number; note: string } {
  ...
  return {
    score,
    note: avg === 0 ? `dialed to ${target}` : `${avg.toFixed(1)} step(s) off ${target}`,
  };
}
...
export function occasionScore(items: ClosetItem[], occ: Occasion): { score: number; note: string } {
  ...
  const blob = items
    .map((i) => `${i.name} ${i.brand ?? ""} ${i.material ?? ""} ${i.fit ?? ""}`.toLowerCase())
    .join(" ");
  ...
  const note =
    kw > 0
      ...
      : bad
        ? `${bad} piece(s) fight the ${occ} vibe`
        : `neutral fit for ${occ}`;
  return { score, note };
}
...
export function generateOutfits(
  ...
  for (const top of tops) {
    for (const bottom of bottoms) {
      const outerCandidates: (ClosetItem | null)[] =
        needsOuter && outers.length ? [...outers] : [null, ...outers.slice(0, 2)];
...
export function pairScent(
  ...
  for (const f of scents) {
    ...
    if (f.season === seasonKey || f.season === "all") {
      s += 0.2;
      reasons.push(`fits ${seasonKey}`);
    }
    if (f.family && preferredFamilies.some((p) => f.family!.toLowerCase().includes(p))) {
      s += 0.2;
      reasons.push(`${f.family} matches ${vibe.toLowerCase()}`);
    }
    if (f.occasions?.includes(ask.occasion)) {
      s += 0.15;
      reasons.push(`tagged for ${ask.occasion}`);
    }
    if (f.projection) {
      ...
      if (formal >= 3 && strong) {
        s += 0.1;
        reasons.push("projection suits formal");
      }
      if (formal <= 1 && soft) {
        s += 0.1;
        reasons.push("soft projection for casual");
      }
    }
    // Tie-breaker: leather top pairs with leather/woody scents. Applied
    // BEFORE picking best so it actually influences the choice.
    if (
      outfit.top.material?.toLowerCase().includes("leather") &&
      f.family?.toLowerCase().includes("leather")
    ) {
      s += 0.05;
